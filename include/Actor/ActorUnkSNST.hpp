//! TODO: This file was generated automatically and might contain errors

#pragma once

#include "Actor/Actor.hpp"
#include "Actor/ActorManager.hpp"
#include "Actor/ActorType.hpp"
#include "global.h"
#include "types.h"

class ActorUnkSNST : public Actor {
public:
    /* 00 (base) */
    /* 94 */ unk32 mUnk_94;

    ActorUnkSNST();

    /* 4C */ virtual ~ActorUnkSNST() override;

    void func_ov026_0211aa28(void);
    void func_ov026_0211aa50(void);
    void func_ov026_0211ab1c(void);
    void func_ov026_0211ab30(void);
    void func_ov026_0211abc0(void);
    void func_ov026_0211ac3c(void);
    void func_ov026_0211ac44(void);
    void func_ov026_0211ac70(void);
    void func_ov026_0211aca4(void);
    void func_ov026_0211acc0(void);
    void func_ov026_0211ace8(void);
    void func_ov026_0211ad18(void);
};

class ActorTypeUnkSNST : public ActorType {
public:
    static ActorTypeUnkSNST gInstance;

    /* 00 (base) */

    ActorTypeUnkSNST();

    /* 0C */ virtual Actor *Create();

    static ActorTypeUnkSNST *GetInstance();
};
