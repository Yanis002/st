//! TODO: This file was generated automatically and might contain errors

#pragma once

#include "Actor/Actor.hpp"
#include "Actor/ActorManager.hpp"
#include "Actor/ActorType.hpp"
#include "global.h"
#include "types.h"

class ActorUnkDMFR : public Actor {
public:
    /* 00 (base) */
    /* 94 */ unk32 mUnk_94;

    ActorUnkDMFR();

    /* 4C */ virtual ~ActorUnkDMFR() override;

    void func_ov088_021705f4(void);
    void func_ov088_0217061c(void);
    void func_ov088_02170690(void);
    void func_ov088_021706a4(void);
    void func_ov088_021706f8(void);
    void func_ov088_02170708(void);
    void func_ov088_0217071c(void);
    void func_ov088_02170730(void);
    void func_ov088_02170780(void);
    void func_ov088_021707a4(void);
    void func_ov088_021707b4(void);
    void func_ov088_021707bc(void);
    void func_ov088_021707d8(void);
    void func_ov088_02170800(void);
    void func_ov088_02170830(void);
    void func_ov088_02170844(void);
    void func_ov088_02170870(void);
    void func_ov088_02170890(void);
};

class ActorTypeUnkDMFR : public ActorType {
public:
    static ActorTypeUnkDMFR gInstance;

    /* 00 (base) */

    ActorTypeUnkDMFR();

    /* 0C */ virtual Actor *Create();

    static ActorTypeUnkDMFR *GetInstance();
};
